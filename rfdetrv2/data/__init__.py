...
def build_roboflow(image_set: str, cfg: Any, resolution: int) -> torch.utils.data.Dataset:
    ...
    if dataset_format == "coco":
        return build_roboflow_from_coco(image_set, cfg, resolution)
    return build_roboflow_from_yolo(image_set, cfg, resolution)


def build_dataset(image_set: str, cfg: Any, resolution: int) -> torch.utils.data.Dataset:
    if cfg.dataset_file == "coco":
        return build_coco(image_set, cfg, resolution)
    if cfg.dataset_file == "o365":
        return build_o365(image_set, cfg, resolution)
    if cfg.dataset_file == "roboflow":
        return build_roboflow(image_set, cfg, resolution)
