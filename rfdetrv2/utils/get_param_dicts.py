...
from rfdetrv2.nn.backbone import Joiner
...
def get_param_dict(cfg: Any, model_without_ddp: nn.Module) -> List[Dict[str, Any]]:
    assert isinstance(model_without_ddp.backbone, Joiner)
    backbone = model_without_ddp.backbone[0]
    backbone_named_param_lr_pairs = backbone.get_named_param_lr_pairs(cfg, prefix="backbone.0")
    ...
    decoder_param_lr_pairs = [
        {"params": param, "lr": cfg.lr * cfg.lr_component_decay}
