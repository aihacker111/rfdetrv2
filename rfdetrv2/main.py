...
# from rfdetrv2.engine import evaluate, train_one_epoch
...
#         # ----------------------------------------------------------------
#         # [GRPO] Khởi tạo GRPOTrainer nếu được bật
#         # Đặt sau build_criterion vì cần criterion.matcher
#         # ----------------------------------------------------------------
#         grpo_trainer = None
#         if getattr(args, "use_grpo", False):
#             from rfdetrv2.rl_grpo import GRPOConfig, GRPOTrainer

#             matcher = getattr(criterion, "matcher", None)
#             if matcher is None:
#                 raise RuntimeError(
#                     "GRPO yêu cầu criterion phải expose `.matcher`. "
#                     "Kiểm tra lại build_criterion_and_postprocessors()."
#                 )

#             grpo_config = GRPOConfig.from_args(args)
#             grpo_trainer = GRPOTrainer(
#                 model=self.model,          # trước DDP wrap
#                 criterion=criterion,
#                 matcher=matcher,
#                 config=grpo_config,
...
#             )
#             logger.info(
#                 "[GRPO] Trainer khởi tạo thành công — "
#                 "G=%d  lambda=%.3f  warmup=%d steps",
#                 grpo_config.num_samples,
#                 grpo_config.lambda_grpo,
#                 grpo_config.warmup_steps,
#             )
#         # ----------------------------------------------------------------
...
#             # [GRPO] Cập nhật ref model sau DDP wrap để chạy trên đúng device
#             if grpo_trainer is not None:
#                 import copy
#                 grpo_trainer._ref_model = copy.deepcopy(model_without_ddp)
#                 for p in grpo_trainer._ref_model.parameters():
#                     p.requires_grad_(False)
#                 grpo_trainer._ref_model.eval()
#                 grpo_trainer._ref_model.to(device)
...
#             args.lr_encoder = args.lr_encoder * world_size
#             if utils.is_main_process():
#                 print(
#                     f'Multi-GPU: scaling LR by world_size={world_size} '
...
#         # ----------------------------------------------------------------
#         # Resume từ checkpoint — bao gồm GRPO state nếu có
#         # ----------------------------------------------------------------
#         if args.resume:
...
#             # [GRPO] Restore GRPO trainer state nếu checkpoint có lưu
#             if grpo_trainer is not None and 'grpo' in checkpoint:
#                 grpo_trainer.load_state_dict(checkpoint['grpo'])
#                 logger.info("[GRPO] Trainer state restored from checkpoint.")
...
#                 # --------------------------------------------------------
#                 # [GRPO] Truyền trainer vào engine — None = supervised only
#                 # --------------------------------------------------------
...
#                     # [GRPO] Lưu ref model để resume training đúng chỗ
#                     if grpo_trainer is not None:
#                         weights['grpo'] = grpo_trainer.state_dict()
...
#                     }
#                     if grpo_trainer is not None:
#                         best_weights['grpo'] = grpo_trainer.state_dict()
...
#                         }
#                         if grpo_trainer is not None:
#                             ema_weights['grpo'] = grpo_trainer.state_dict()
...
#     parser.add_argument('--projector_scale',     default='P4', type=str, nargs='+',
#                         choices=('P3', 'P4', 'P5', 'P6'))
...
#                         choices=['step', 'cosine', 'linear', 'multistep'])
...
#     # ----------------------------------------------------------------
#     # [GRPO] RL training args
#     # Giữ use_rl / rl_weight / rl_reward_type để không break cũ
#     # ----------------------------------------------------------------
#     parser.add_argument(
#         '--use_rl', action='store_true',
#         help='[DEPRECATED] Dùng --use_grpo thay thế.',
#     )
#     parser.add_argument(
#         '--rl_weight', default=0.5, type=float,
#         help='[DEPRECATED] Ignored khi use_grpo=True.',
#     )
#     parser.add_argument(
#         '--rl_reward_type', default='hungarian_full', type=str,
#         choices=['hungarian', 'hungarian_full', 'recall_50', 'recall_75', 'combined', 'mean_iou'],
#         help='[DEPRECATED] Dùng --grpo_reward_type thay thế.',
#     )

#     # GRPO
#     parser.add_argument(
#         '--use_grpo', action='store_true',
#         help='Bật GRPO training (supervised + policy gradient).',
#     )
#     parser.add_argument(
#         '--grpo_num_samples', default=4, type=int,
#         help='Số diverse samples G per image trong GRPO.',
#     )
#     parser.add_argument(
#         '--grpo_lambda', default=0.1, type=float,
#         help='Weight của L_grpo: L_total = L_sup + lambda * L_grpo.',
#     )
#     parser.add_argument(
#         '--grpo_clip_eps', default=0.2, type=float,
#         help='PPO clip epsilon.',
#     )
#     parser.add_argument(
#         '--grpo_warmup_steps', default=500, type=int,
#         help='Số bước supervised-only trước khi bật GRPO.',
#     )
#     parser.add_argument(
#         '--grpo_reward_type', default='hungarian_full', type=str,
#         choices=['hungarian', 'hungarian_full', 'recall_50', 'recall_75', 'combined'],
#         help='Loại reward để tính advantage trong GRPO.',
#     )
#     parser.add_argument(
#         '--grpo_score_thresh', default=0.3, type=float,
#         help='Confidence threshold cho recall/precision trong reward.',
#     )
#     parser.add_argument(
#         '--grpo_query_noise_std', default=0.02, type=float,
#         help='Std của noise thêm vào object queries khi sampling.',
#     )
#     parser.add_argument(
#         '--grpo_ref_ema_decay', default=0.999, type=float,
#         help='EMA decay cho ref model.',
#     )
#     parser.add_argument(
#         '--grpo_advantage_clip', default=5.0, type=float,
#         help='Clip advantage để tránh extreme gradient.',
#     )
#     parser.add_argument(
#         '--grpo_ref_update_interval', default=10, type=int,
#         help='Ref model recompute mỗi N bước. =1: mỗi bước (chậm, chính xác). =10: nhanh hơn.',
#     )
#     parser.add_argument(
#         '--grpo_every_n_steps', default=4, type=int,
#         help='Tính GRPO 1 lần mỗi N bước (N=4 recommended). Supervised vẫn chạy mọi bước.',
#     )
#     parser.add_argument(
#         '--grpo_freeze_backbone', default=True, type=lambda x: x.lower() != 'false',
#         help='Freeze DINOv3 backbone trong GRPO grad pass để tiết kiệm compute.',
#     )
#     parser.add_argument(
#         '--grpo_use_ref_model', default=False, action='store_true',
#         help='Dùng ref model (PPO ratio). False = REINFORCE thuần, nhanh hơn.',
#     )
#     # ----------------------------------------------------------------
...
#     use_rl=False,
#     rl_weight=0.5,
#     rl_reward_type='hungarian_full',
#     use_grpo=False,
#     grpo_num_samples=2,
#     grpo_lambda=0.1,
#     grpo_clip_eps=0.2,
#     grpo_warmup_steps=500,
#     grpo_reward_type='hybrid',
#     grpo_score_thresh=0.3,
#     grpo_query_noise_std=0.02,
#     grpo_ref_ema_decay=0.999,
#     grpo_advantage_clip=5.0,
#     grpo_ref_update_interval=10,
#     grpo_every_n_steps=4,
#     grpo_freeze_backbone=True,
#     grpo_use_ref_model=False,
#     # ----------------------------------------------------------------
#     # Additional
#     subcommand=None,
#     **extra_kwargs,
...
#         grpo_warmup_steps=grpo_warmup_steps,
#         grpo_reward_type=grpo_reward_type,
#         grpo_score_thresh=grpo_score_thresh,
#         grpo_query_noise_std=grpo_query_noise_std,
#         grpo_ref_ema_decay=grpo_ref_ema_decay,
#         grpo_advantage_clip=grpo_advantage_clip,
#         grpo_ref_update_interval=grpo_ref_update_interval,
#         grpo_every_n_steps=grpo_every_n_steps,
#         grpo_freeze_backbone=grpo_freeze_backbone,
#         grpo_use_ref_model=grpo_use_ref_model,
