...
import rfdetrv2.datasets.transform_album as T
...
def make_coco_transforms(
    ...
    if image_set == 'train':
        doc_aug = [T.DocumentAugmentation(apply_spatial=True, p=0.5)] if document_aug else []
        return T.Compose([
...
def resolve_coco_annotation_path_for_categories(
    root: Union[str, Path]
) -> Optional[Path]:
    root = Path(root)
    candidates = [
